...
public enum TextContents {
	WELCOME("欢迎来到JoyMaker！！！"), DEVELOPING("开发中，敬请期待！"), SUBMIT_BASIC("请提交基本信息"), MENU_ROOT(
			...
			"您的可用积分为： "), Change_CREDITS_INFO("您添加的积分为："), INTRODUCE_AWARD_INFO(
			...
			"充值卡"), EXCHAGE_INFO(
			"请输入您想兑换的奖品信息，输入格式为：兑换[卡面额]元[奖品类型][兑换数量]张，例如\"兑换50元超市购物卡2张\""), EXCHANGE_VALUE_WRONG(
			"兑换金额不合法，只有20、50、100元三种面额！"), EXCHANGE_CREDIT_WRONG("没有足够的积分！"), EXCHANGE_NUMBER_WRONG("没有足够数量的卡！"), EXCHANGE_OK(
			"恭喜您，兑换成功！您兑换的结果为"), EXCHANGE_QUERY_INFO("您的兑换记录如下所示：\n\r"), EXCHANGE_QUERY_NULL("您没有兑换过任何奖品，"
					+ "快点击\"兑换奖品\"来兑换吧！"), CREDITS_RANKING_INFO(
			"积分排行榜为(名次、用户id、积分)：\n\r"), BASICINFO("基本信息"), IMAGE("图片"), VOICE(
			"语音"), LOCATION("位置"), DESCRIPTIONS("描述文字"), PROVIDE_MORE(
			"为了提高事件处理的便利，请提交XXX信息；若不需要提交请再次点击结束事件按钮！"), ADD_AWARDS("成功添加了超市购物卡、公交一卡通、手机充值卡三种卡的20、50"
					+ "、100面额各一张！");
